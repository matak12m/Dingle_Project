...
define n = Character("", what_prefix = "{i}", what_suffix = "{/i}", window_background=Frame("gui/thoughtbox.png"))
...
label start:
    ...
    u "{cps=*0.4}Okay, um. I'm just gonna go then.{/cps} {w=0.3}  {i} Gosh this is awkward...{/i}"
    with vpunch
    player "Wait!"
    
    n  "They seem friendly enough, and you're actually kind of curious"
    show an phiast unhappy: 
        xalign 2.5 yalign 1.1
    ...
    show fungie annoyed at offscreen_right

    show fungie annoyed:
        xalign 2.0 yalign -1.1
    ...
    label choice_prologue_1_leave:
        ...
        show fungie annoyed
        f "Very well.{w=0.4} enjoy your time with the sea monster. Everyone knows they're {shader=jitter:u__jitter=0.0, 3.0}{cps=*0.5}{i}soooo{/i}{/cps} {/shader} inteligent."
        show an phiast angry:
            easein 0.1 yalign 1.0
        with vpunch
        a "Hey!" 
        ...
        show fungie smug
        f "There's only one mascot in Dingle, and that is {shader=jitter:u__jitter=0.0, 3.0}{i}Me!{/i}{/shader}"
        show fungie annoyed
